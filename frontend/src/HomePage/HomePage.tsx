...
interface HomePageState {
  ...
  isCloningApp: boolean;
  isExportingApp: boolean;
  isImportingApp: boolean;
  isDeletingAppFromFolder: boolean;
  currentFolder?: Folder;
  currentPage: number;
  appSearchKey: string;
  appToBeDeleted?: Application;
  showAppDeletionConfirmation: boolean;
  showRemoveAppFromFolderConfirmation: boolean;
  showAddToFolderModal: boolean;
  ...
  appOperations: ApplicationOperation;
  showTemplateLibraryModal: boolean;
  app?: Application;
    ...
    folderService.getAll(appSearchKey).then((data) => {
      const currentFolder = data?.folders?.filter(
        (folder) =>
          ...
          folder.id === this.state.currentFolder?.id
      )?.[0];
      this.setState({
    ...
    switch (action) {
      case "create":
        permissionGrant = this.canAnyGroupPerformAction(
          "app_create",
          currentSession.group_permissions
        );
        break;
      case "read":
      case "update":
        permissionGrant =
          this.canAnyGroupPerformActionOnApp(
            action,
            currentSession.app_group_permissions,
            app
        ...
        <div className="wrapper home-page">
          ...
          {isExportingApp && app.hasOwnProperty("id") && (
            <ExportAppModal
              show={isExportingApp}
              closeModal={() => {
                this.setState({ isExportingApp: false, app: null });
              }}
