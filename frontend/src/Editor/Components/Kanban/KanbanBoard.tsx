...
export function KanbanBoard({ widgetHeight, kanbanProps, parentRef }) {
  ...
  const columnDataAsObj = useMemo(
    () => convertArrayToObj(columnData),
    [JSON.stringify(columnData)]
  );

  // eslint-disable-next-line react-hooks/exhaustive-deps
  const cardDataAsObj = useMemo(
    () => convertArrayToObj(cardData),
  ...
  useEffect(() => {
    droppableItemsColumnId.current = containers.find(
      (container) => items[container]?.length > 0
    );
  }, [items, containers]);
  ...
  registerAction(
    ...
    async function (cardId, columnId) {
      ...
      setExposedVariable("lastCardMovement", lastCardMovement).then(() =>
  ...
  registerAction(
    ...
    async function (cardDetails) {
      ...
      if (
        cardDetails?.columnId === undefined ||
        items[cardDetails?.columnId] === undefined
      )
  ...
  const onDragOver = ({ active, over }) => {
    ...
    if (
      overId == null ||
      overId === TRASH_ID ||
      activeIdInString.includes("tj-kanban-container-")
    ) {
      return;
    }
  ...
  const onDragEnd = ({ active, over }) => {
    ...
    if (overContainer) {
      ...
      if (activeIndex !== overIndex) {
        ...
        if (
          cardDetails?.id === over?.id &&
          destinationColumnId === overContainer
        ) {
          shouldUpdateData.current = true;
          setExposedVariable("lastCardMovement", {
            ...cardMovementRef.current,
          }).then(() => {
    ...
    <>
      ...
      >
        ...
        >
                    ...
                    {items[columnId].map((value, index) => {
                      return (
                        <SortableItem
                          ...
                          isFirstItem={
                            index === 0 &&
                            droppableItemsColumnId.current === columnId
                          }
          ...
          <button
            className={cx(
              "kanban-add-card-button jet-button btn",
              !enableAddCard && "invisible"
            )}
