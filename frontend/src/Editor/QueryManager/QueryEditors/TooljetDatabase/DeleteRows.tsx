...
export const DeleteRows = React.memo<{ darkMode?: boolean }>(({ darkMode }) => {
  const {
    columns,
  ...
  function addNewFilterConditionPair() {
    const existingFilters = deleteRowsOptions?.where_filters
      ? Object.values(deleteRowsOptions?.where_filters)
      : [];
    ...
    <div className="tab-content-wrapper tj-db-field-wrapper mt-2">
      <div className="d-flex">
        ...
        <div className="field-container flex-grow-1  mb-2">
          {Object.values(deleteRowsOptions?.where_filters || {}).map(
            (filter: any) => (
              <RenderFilterFields
                key={filter.id}
                {...filter}
          ...
          <ButtonSolid
            ...
            className={
              isEmpty(deleteRowsOptions?.where_filters || {}) ? "" : "mt-2"
            }
          >
            <svg
              width="11"
