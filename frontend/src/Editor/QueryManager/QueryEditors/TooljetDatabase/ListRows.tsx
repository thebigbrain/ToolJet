...
export const ListRows = React.memo<{ darkMode?: boolean }>(({ darkMode }) => {
  ...
  function addNewFilterConditionPair() {
    const existingFilters = listRowsOptions?.where_filters
      ? Object.values(listRowsOptions?.where_filters)
      : [];
  ...
  function addNewSortConditionPair() {
    const existingFilters = listRowsOptions?.order_filters
      ? Object.values(listRowsOptions?.order_filters)
      : [];
    const emptyFilter = { column: "", order: "" };
    const newFilter = { ...emptyFilter, ...{ id: uniqueId() } };
    handleOrderFiltersChange({
      ...existingFilters,
      ...{ [newFilter.id]: newFilter },
    });
  }
        ...
        <div className="tab-content-wrapper">
          <div className="d-flex mb-2">
            ...
            <div className="field-container flex-grow-1">
              {Object.values(listRowsOptions?.where_filters || {}).map(
                (filter: any) => (
                  <RenderFilterFields
                    key={filter.id}
                    {...filter}
              ...
              <ButtonSolid
                ...
                className={
                  isEmpty(listRowsOptions?.where_filters || {}) ? "" : "mt-2"
                }
              >
                <svg
                  width="11"
          ...
          <div className="fields-container d-flex mb-2">
            ...
            <div className="field-container flex-grow-1">
              {Object.values(listRowsOptions?.order_filters || {}).map(
                (filter: any) => (
                  <RenderSortFields
                    key={filter.id}
                    {...filter}
              ...
              <ButtonSolid
                ...
                className={
                  isEmpty(listRowsOptions?.order_filters || {}) ? "" : "mt-2"
                }
              >
                <svg
                  width="11"
...
const RenderSortFields = ({
  ...
  const existingColumnOptions = Object.values(
    listRowsOptions?.order_filters
