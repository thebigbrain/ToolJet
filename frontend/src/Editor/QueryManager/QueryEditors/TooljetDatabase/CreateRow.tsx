...
export const CreateRow = React.memo<{
  optionchanged?;
  options?;
  darkMode?: boolean;
}>(({ optionchanged, options, darkMode }) => {
    ...
    <div className="row tj-db-field-wrapper">
      <div className="tab-content-wrapper mt-2 d-flex">
        ...
        <div className="field-container flex-grow-1">
          {Object.entries<any>(columnOptions).map(([key, value]) => (
            <RenderColumnOptions
              key={key}
...
const RenderColumnOptions = ({
  ...
  if (existingColumnOption.length > 0) {
    displayColumns = displayColumns.filter(
      ({ value }) =>
        !existingColumnOption
          .map((item: any) => item.column !== column && item.column)
          .includes(value)
    );
  }
