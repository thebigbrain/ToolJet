import React, { useState, forwardRef, useRef, useEffect, Ref } from "react";
...
type QueryManagerHeaderProps = {
  darkMode?: boolean;
  options?;
  editorRef?;
};

export const QueryManagerHeader = forwardRef<any, QueryManagerHeaderProps>(
  ({ darkMode, options, editorRef }, ref) => {
    const { renameQuery } = useDataQueriesActions();
    const selectedQuery = useSelectedQuery();
    const selectedDataSource = useSelectedDataSource();
    const [showCreateQuery, setShowCreateQuery] = useShowCreateQuery();
    const queryName = selectedQuery?.name ?? "";
    const { queries } = useCurrentState();
    ...
    const renderRunButton = () => {
      ...
      return (
        ...
        >
          ...
          {isInDraft && (
            <Tooltip id="query-header-btn-run" className="tooltip" />
          )}
        </span>
      );
    };
...
const PreviewButton = ({ buttonLoadingState, onClick }) => {
  ...
  return (
    <button
      onClick={onClick}
      className={`default-tertiary-button float-right1 ${buttonLoadingState(
        previewLoading
      )}`}
...
const NameInput = ({ onInput, value, darkMode }) => {
  const [isFocussed, setIsFocussed] = useNameInputFocussed();
  const [name, setName] = useState(value);
  const isVersionReleased = useAppVersionStore(
    (state) => state.isVersionReleased
  );
        ...
        {isFocussed ? (
          <input
            ...
            onKeyDown={(event) => {
              ...
              if (event.keyCode === 13) {
                setIsFocussed(false);
                handleInput((event.target as any).value);
              }
            }}
            onBlur={({ target }) => {
              setIsFocussed(false);
