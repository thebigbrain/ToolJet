...
const useDebugger = ({ currentPageId, isDebuggerOpen }) => {
  ...
  useEffect(() => {
    if (currentPageId) {
      const olderPageErrorFromHistory =
        errorHistory.pageLevel[currentPageId] ?? [];
      const olderAppErrorFromHistory = errorHistory.appLevel ?? [];
      setErrorLogs(() => [
        ...olderPageErrorFromHistory,
        ...olderAppErrorFromHistory,
      ]);
    }
  ...
  useEffect(() => {
    ...
    const newPageLevelErrorLogs = newErrorLogs.filter(
      (error) => error.strace === "page_level"
    );
    const newAppLevelErrorLogs = newErrorLogs.filter(
      (error) => error.strace === "app_level"
    );
    if (newErrorLogs) {
      ...
      setErrorHistory((prevErrors) => {
        ...
        return {
          ...
          pageLevel: {
            [currentPageId]: [
              ...newPageLevelErrorLogs,
              ...(copy.pageLevel[currentPageId] ?? []),
            ],
          },
        };
      });
  ...
  useEffect(() => {
    const successQueryLogs =
      debuggerActions.generateQuerySuccessLogs(succededQuery);
    if (successQueryLogs?.length) {
      setAllLog((prevLogs) => {
        const temp = [...successQueryLogs, ...prevLogs];
        const sortedDatesDesc = temp.sort((a, b) =>
