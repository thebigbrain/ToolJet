  ...
  useEffect(() => {
    ...
    if (!isEmpty(dataSourcesForFilters)) {
      ...
      filteredDataQueries = dataQueries.filter((query) => {
        const queryDSId = excludedDataSources.includes(query.data_source_id)
          ? null
          : query.data_source_id;
        return dataSourcesForFilters.some(
          (source) => source.id == queryDSId && source.kind === query.kind
        );
      });
    }
      ...
      >
        <div className="queries-header row d-flex align-items-center justify-content-between">
          <div className="col-auto d-flex">
            ...
            <button
              onClick={() => {
                showSearchBox && setSearchTermForFilters(null);
            ...
            >
              <Search width="14" fill="var(--slate12)" />
            </button>
            <FilterandSortPopup
        ...
        <div
          className={cx(
            "queries-header row d-flex align-items-center justify-content-between",
            {
        ...
        >
          <div className="col-auto w-100">
            <div
              className={`queries-search d-flex ${darkMode && "theme-dark"}`}
            >
              <SearchBox
                ref={searchBoxRef}
