...
export const EventManager = ({
  ...
  if (checkIfClicksAreInsideOf) {
    document.body.addEventListener("click", function (event) {
      if (checkIfClicksAreInsideOf.contains(event.target as HTMLElement)) {
  ...
  const actionLookup = Object.fromEntries(
    ActionTypes.map((actionType) => [actionType.id, actionType])
  );
  ...
  function getComponentOptions(componentType = "") {
    ...
    Object.keys(components || {}).forEach((key) => {
      if (
        componentType === "" ||
        components[key].component.component === componentType
      ) {
  ...
  function getComponentOptionsOfComponentsWithActions(componentType = "") {
    ...
    Object.keys(components || {}).forEach((key) => {
      const targetComponentMeta = componentTypes.find(
        (componentType) =>
          components[key].component.component === componentType.component
      );
  ...
  function getComponentActionOptions(componentId) {
    ...
    const filteredComponents = Object.entries(components ?? {}).filter(
      ([key, _value]) => key === componentId
    );
    if (_.isEmpty(filteredComponents)) return [];
    const component = filteredComponents[0][1];
    const targetComponentMeta = componentTypes.find(
      (componentType) =>
        component.component.component === componentType.component
    );
        ...
        <Popover.Body>
          ...
          {actionLookup[event.actionId]?.options?.length > 0 && (
            <div className="hr-text" data-cy="action-option">
              {t(
                "editor.inspector.eventManager.actionOptions",
                "Action options"
              )}
            </div>
          )}
            ...
            {event.actionId === "copy-to-clipboard" && (
              <div className="p-1">
                ...
                <CodeHinter
                  ...
                  onChange={(value) =>
                    handlerChanged(index, "contentToCopy", value)
                  }
                  usePortalEditor={false}
                />
              </div>
                    ...
                    <Select
                      ...
                      onChange={(value) => {
                        ...
                        handlerChanged(
                          ...
                          getComponentActionDefaultParams(
                            event?.componentId,
                            value
                          )
                        );
                      ...
                      {param.type === "select" ? (
                        ...
                        >
                          <Select
                            ...
                            value={valueForComponentSpecificActionHandle(
                              event,
                              param
                            )}
                            search={true}
                        ...
                        <div
                          className={`${
                            ...
                            param.type == "select" && "component-action-select"
                          }`}
                        ...
                        >
                          <CodeHinter
                            theme={darkMode ? "monokai" : "default"}
                            mode="javascript"
                            initialValue={valueForComponentSpecificActionHandle(
                              event,
                              param
                            )}
                            onChange={(value) => {
          ...
          {({ innerRef, droppableProps, placeholder }) => (
            <div {...droppableProps} ref={innerRef}>
              {events.map((event, index) => {
                const actionMeta = ActionTypes.find(
                  (action) => action.id === event.actionId
                );
                const rowClassName = `card-body p-0 ${
                  focusedEventIndex === index ? " bg-azure-lt" : ""
                }`;
                return (
                  <Draggable
                    key={index}
                    ...
                    {renderDraggable((provided, snapshot) => {
                      ...
                      return (
                        <OverlayTrigger
                          ...
                          onHide={(() => setFocusedEventIndex(null)) as never}
                          onToggle={(showing) => {
                            if (showing) {
                            ...
                            <div className="card column-sort-row border-0 bg-slate2">
                              ...
                              >
                                <div className="row p-2" role="button">
                                  ...
                                  >
                                    {
                                      componentMeta.events[event.eventId][
                                        "displayName"
                                      ]
                                    }
                                  </div>
    ...
    return (
      <div className={`mb-3 ${events.length === 0 ? "" : "mt-2"}`}>
        <ButtonSolid
          ...
          onClick={addHandler}
          data-cy={
            events.length === 0 ? "add-event-handler" : "add-more-event-handler"
          }
        >
          <AddRectangle
            width="15"
