...
const ManageOrgConstantsComponent = ({ darkMode }) => {
  ...
  const [showConstantDeleteConfirmation, setShowConstantDeleteConfirmation] =
    useState(false);
  ...
  const updateActiveEnvironmentTab = (environment, allConstants = []) => {
    ...
    const envName = activeTabEnvironment
      ? activeTabEnvironment.name
      : environment.name;
  ...
  const updateTableData = (
    ...
    const constantsForEnvironment = orgContants
      .filter((constant) => {
        const envConstant = constant?.values.find(
          (value) => value.environmentName === envName
        );
    ...
    if (orgConstants?.constants?.length > 1) {
      orgConstants.constants.sort((a, b) => {
        return new Date(b.createdAt)
          .getTime()
          .toString()
          .localeCompare(new Date(a.createdAt).getTime().toString());
      });
    }

    setConstants(orgConstants?.constants);

    let orgEnvironments: any = await fetchEnvironments();
    setEnvironments(orgEnvironments?.environments);
    const currentEnvironment = orgEnvironments?.environments?.find(
      (env) => env?.is_default === true
    );
  ...
  const checkIfConstantNameExists = (name, environementId?) => {
    ...
    const envConstants = constants.filter((constant) => {
      return constant.values.some(
        (value) => value.id === environementId && value.value !== ""
      );
    });
  ...
  const createOrUpdate = (variable, shouldUpdate = false) => {
    ...
    const shouldUpdateConstant =
      mode === "edit" && shouldUpdate ? true : constantExistsInDiffEnv;
  ...
  const findValueForEnvironment = (constantValues, environmentName) => {
    ...
    const value = constantValues?.find(
      (value) => value.environmentName === environmentName
    );
    return value?.value;
  };

  useEffect(() => {
    fetchConstantsAndEnvironments();
  ...
  const confirmMessage = (
    <span>
      Are you sure you want to delete <b>{selectedConstant?.name}</b> from{" "}
      <b>{activeTabEnvironment?.name}</b>?
    </span>
  );
  return (
...
const RenderEnvironmentsTab = ({
  ...
  const constantCount = (constants, envId) => {
    const envConstant = constants
      .map((constant) =>
        constant.values.filter((v) => v.id === envId && v.value !== "")
      )
