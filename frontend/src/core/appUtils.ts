...
import { ERROR_TYPES } from "@externals/helpers/appUtils";
...
async function executeRunPycode(_ref, code, query, isPreview, mode) {
  ...
  const evaluatePythonCode = async (pyodide) => {
    ...
    try {
      ...
      const actions: any = generateAppActions(_ref, query.id, mode, isPreview);
  ...
  return { data: await evaluatePythonCode(pyodide) };
}
...
async function exceutePycode(payload, code, currentState, query, mode) {
  ...
  return await evaluatePython(pyodide);
}
    ...
    switch (event.actionId) {
      ...
      case "run-query": {
        ...
        if (params) {
          Object.keys(params).map(
            (param) =>
              (resolvedParams[param] = resolveReferences(
                params[param],
                getCurrentState(),
                undefined
              ))
      ...
      case "go-to-app": {
        ...
        const queryParams = event.queryParams?.reduce(
          (result, queryParam) => ({
            ...result,
            ...{
              [resolveReferences(queryParam[0], getCurrentState())]:
                resolveReferences(
                  queryParam[1],
                  getCurrentState(),
                  undefined,
      ...
      case "control-component": {
        const component = Object.values(
          getCurrentState()?.components ?? {}
        ).filter((component: any) => component.id === event.componentId)[0];
...
export function getQueryVariables(options, state) {
  ...
  switch (optionsType) {
    case "string": {
      ...
      if (
        options.match(/\{\{(.*?)\}\}/g)?.length > 1 &&
        options.includes("{{constants.")
      ) {
...
export function previewQuery(
  ...
  const { setPreviewLoading, setPreviewData } =
    useQueryPanelStore.getState().actions;
...
export function runQuery(
  ...
  const query = useDataQueriesStore
    .getState()
...
export function setTablePageIndex(_ref, tableId, index) {
  ...
  const table: any = Object.entries(getCurrentState().components).filter(
    (entry: any) => entry[1].id === tableId
  )[0][1];
...
export const getSvgIcon = (
  ...
  if (key === "runjs")
    return React.createElement(RunjsIcon, { style: { height, width } });
  if (key === "tooljetdb") return React.createElement(RunTooljetDbIcon);
  if (key === "runpy") return React.createElement(RunPyIcon);
  const Icon = allSvgs[key];

  if (!Icon) return React.createElement(React.Fragment);

  return React.createElement(Icon, { style: { height, width, ...styles } });
};
...
export const debuggerActions = {
  ...
  generateQuerySuccessLogs: (logs) => {
    ...
    Object.entries<any>(logs).forEach(([key, value]) => {
      const generalProps = {
        key,
        type: value.type,
        page: value.page,
...
export const getComponentName = (currentState, id) => {
  try {
    const name = Object.entries<any>(currentState?.components).filter(
...
const getSelectedText = () => {
  ...
  if (window.document.getSelection) {
    navigator.clipboard.writeText(window.document.getSelection().toString());
  }
...
export async function executeMultilineJS(
  ...
  const queryDetails = useDataQueriesStore
    .getState()
    .dataQueries.find((q) => q.id === queryId);
  hasParamSupport = !hasParamSupport
    ? queryDetails?.options?.hasParamSupport
    : hasParamSupport;
  ...
  if (queryDetails) {
    Object.keys(defaultParams).map((key) => {
      ...
      formattedParams[key] =
        parameters[key] === undefined ? defaultParams[key] : parameters[key];
    });
  } else {
    ...
    currentState.queries[key] = {
      ...
      run: (params) => {
        ...
        query.options.parameters?.forEach(
          (arg) => (processedParams[arg.name] = params[arg.name])
        );
...
export const generateAppActions = (_ref, queryId, mode, isPreview = false) => {
  ...
  const currentComponents: any = _ref.state?.appDefinition?.pages[currentPageId]
    ?.components
  ...
  const runQuery = (queryName = "", parameters) => {
    ...
    if (!_.isEmpty(query?.options?.parameters)) {
      query.options.parameters?.forEach(
        (param) =>
          parameters && (processedParams[param.name] = parameters?.[param.name])
      );
    }
  ...
  const generateFile = (fileName, fileType, data) => {
    if (!fileName || !fileType || !data) {
      return toast.error(
        "Action failed: fileName, fileType and data are required"
      );
    }
  ...
  const switchPage = (pageHandle, queryParams = []) => {
    ...
    const pageId = Object.keys(pages).find(
      (key) => pages[key].handle === pageHandle
    );
