...
import { buildURLWithQuery } from "@externals/helpers/utils";
...
export default function GoogleSSOLoginButton(props) {
  ...
  const googleLogin = (e) => {
    ...
    const authUrl = buildURLWithQuery(
      "https://accounts.google.com/o/oauth2/auth",
      {
        redirect_uri: `${config?.TOOLJET_HOST}${
          config?.SUB_PATH ?? "/"
        }sso/google${props.configId ? `/${props.configId}` : ""}`,
