      ...
      <div className="tj-user-table-wrapper">
        <div className="card-table fixedHeader table-responsive  ">
          <table
            ...
            className="users-table table table-vcenter h-100"
          >
            <thead>
              <tr>
                <th data-cy="users-table-name-column-header">
                  {translator(
                    "header.organization.menus.manageUsers.name",
                    "Name"
                  )}
                </th>
                <th data-cy="users-table-email-column-header">
                  {translator(
                    "header.organization.menus.manageUsers.email",
                    "Email"
                  )}
                </th>
                {users && users[0]?.status ? (
                  <th data-cy="users-table-status-column-header">
                    {translator(
                      "header.organization.menus.manageUsers.status",
                      "Status"
                    )}
                  </th>
                ) : (
                      ...
                      {user.status && (
                        <td className="text-muted">
                          ...
                          <small
                            ...
                            data-cy={`${user.name
                              .toLowerCase()
                              .replace(/\s+/g, "-")}-user-status`}
                          >
                            {user.status}
                          </small>
                          {user.status === "invited" &&
                          "invitation_token" in user ? (
                            <div className="workspace-clipboard-wrap">
                              ...
                              <Tooltip
                                id="tooltip-for-copy-invitation-link"
                                className="tooltip"
                              />
                            </div>
                          ) : (
                        ...
                        <ButtonSolid
                          ...
                          onClick={() => {
                            user.status === "archived"
                              ? unarchiveOrgUser(user.id)
                              : archiveOrgUser(user.id);
                          }}
                          data-cy="button-user-status-change"
                        >
                          {user.status === "archived"
                            ? translator(
                                "header.organization.menus.manageUsers.unarchive",
                                "Unarchive"
                              )
                            : translator(
                                "header.organization.menus.manageUsers.archive",
                                "Archive"
                              )}
                        </ButtonSolid>
                      </td>
