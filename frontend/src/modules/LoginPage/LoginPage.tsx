...
class LoginPageComponent extends React.Component<LoginProps, LoginState> {
  ...
  componentDidMount() {
    ...
    this.currentSessionObservable =
      authenticationService.currentSession.subscribe((newSession) => {
        if (newSession?.current_organization_name)
          this.setState({
            current_organization_name: newSession.current_organization_name,
          });
        ...
        if (response.data.statusCode === 422) {
          authenticationService
            ...
            .then(({ current_organization_id }) => {
              ...
              this.props.navigate("/switch-workspace");
            })
            .catch(() => {
              navigateTo("/login");
            });
  ...
  componentWillUnmount() {
    this.currentSessionObservable &&
      this.currentSessionObservable.unsubscribe();
  }

  handleChange = (event) => {
    const name = event.target.name;
    const partialState: LoginState = { emailError: "" };
  ...
  authSuccessHandler = () => {
    ...
    if (from.pathname !== "/confirm")
      ...
      from.pathname = `${returnWorkspaceIdIfNeed(from.pathname)}${
  ...
  render() {
    const { isLoading, configs, isGettingConfigs, navigateToLogin } =
      this.state;
    return (
      <>
        {navigateToLogin ? (
