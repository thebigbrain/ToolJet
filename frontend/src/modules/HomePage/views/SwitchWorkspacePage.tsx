...
import { getAvatar, appendWorkspaceId } from "@externals/helpers/utils";
...
function SwitchWorkspaceModal({ organizations, switchOrganization, ...props }) {
  ...
  return (
    <Modal
      {...props}
      className={`organization-switch-modal ${
    ...
    >
      ...
      <Modal.Footer>
        <button
          ...
          onClick={() => switchOrganization(selectedOrganizationId)}
        >
          {t(
            "globals.workspace-modal.continue-btn",
            "Continue on this workspace"
          )}
        </button>
      </Modal.Footer>
    </Modal>
...
export function SwitchWorkspacePage({ darkMode }) {
