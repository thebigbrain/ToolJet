...
const Sort = ({
    ...
    >
      <Popover.Body bsPrefix="storage-filter-popover">
        <div className="card-body" data-cy="sort-card-body">
          {Object.values(filters).map((filter: any, index) => {
            return (
              <SortForm
                {...filter}
                key={index}
  ...
  const checkIsFilterObjectEmpty = (filter) =>
    !isEmpty(filter.column) && !isEmpty(filter.order);
  const areFiltersApplied =
    !show && Object.values(filters).some(checkIsFilterObjectEmpty);
  ...
  return (
    ...
    >
      <button
        className={cx(
          "border-0 tj-db-sort-btn tj-text-xsm font-weight-500 ghost-black-operation",
          {
      ...
      >
        ...
        {areFiltersApplied && (
          <span>
            ed by{" "}
            {pluralize(
              Object.values(filters).filter(checkIsFilterObjectEmpty).length,
              "column"
            )}
          </span>
        )}
