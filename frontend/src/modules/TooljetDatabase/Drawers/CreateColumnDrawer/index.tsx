  ...
  return (
    <>
      <button
        ...
        className={`add-new-column-btn ghost-black-operation ${
          isCreateColumnDrawerOpen && "open"
        }`}
      ...
      >
        <CreateColumnForm
          onCreate={() => {
            ...
            tooljetDatabaseService
              .findOne(organizationId, selectedTable)
              .then(({ data = [], error }) => {
                if (error) {
                  toast.error(
                    error?.message ?? `Failed to fetch table "${selectedTable}"`
                  );
                  return;
                }
