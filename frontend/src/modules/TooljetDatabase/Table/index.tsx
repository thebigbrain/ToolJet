...
const Table = ({ openCreateRowDrawer, openCreateColumnDrawer }) => {
  ...
  const fetchTableMetadata = () => {
    tooljetDatabaseService
      .viewTable(organizationId, selectedTable)
      .then(({ data = [], error }) => {
        if (error) {
          toast.error(
            error?.message ??
              `Error fetching metadata for table "${selectedTable}"`
          );
          return;
        }
  ...
  const fetchTableData = (queryParams = "", pagesize = 50, pagecount = 1) => {
    ...
    tooljetDatabaseService
      .findOne(organizationId, selectedTable, params)
      .then(({ headers, data = [], error }) => {
        setLoading(false);
        if (error) {
          toast.error(
            error?.message ?? `Error fetching table "${selectedTable}" data`
          );
          return;
        }
        const totalContentRangeRecords =
  ...
  const handleDeleteRow = async () => {
    ...
    if (shouldDelete) {
      ...
      const { error } = await tooljetDatabaseService.deleteRow(
        organizationId,
        selectedTable,
        query
      );
      ...
      if (error) {
        toast.error(
          error?.message ?? `Error deleting rows from table "${selectedTable}"`
        );
        return;
      }
  ...
  const handleDeleteColumn = async (columnName) => {
    ...
    if (shouldDelete) {
      const { error } = await tooljetDatabaseService.deleteColumn(
        organizationId,
        selectedTable,
        columnName
      );
      if (error) {
        toast.error(
          error?.message ??
            `Error deleting column "${columnName}" from table "${selectedTable}"`
        );
        return;
      }
