...
function DataSourceSelect({
  ...
  useEffect(() => {
    setUserDefinedSourcesOpts(
      Object.entries(groupBy(userDefinedSources, "kind")).map(
        ([kind, sources], index) => ({
          label: (
            <div>
              {index === 0 && (
              ...
              <span className="ms-1 small">
                {dataSourcesKinds.find((dsk) => dsk.kind === kind)?.name ||
                  kind}
              </span>
            </div>
          ),
  ...
  return (
    <div>
      <Select
        onChange={({ source }: any = {}) => handleChangeDataSource(source)}
        classNames={{
          menu: () => "tj-scrollbar",
        }}
        ref={selectRef}
        ...
        styles={{
          ...
          option: (style, { data: { isNested }, isFocused, isDisabled }) => ({
            ...
            cursor: "pointer",
            backgroundColor:
              isFocused && !isNested ? "var(--slate4)" : "transparent",
            ...(isNested
              ? {
                  padding: "0 8px",
                  marginLeft: "19px",
        ...
        onInputChange={() => {
          const queryDsSelectMenu = document.getElementById(
            "query-ds-select-menu"
          );
          if (queryDsSelectMenu && !queryDsSelectMenu?.style?.height) {
            queryDsSelectMenu.style.height =
              queryDsSelectMenu.offsetHeight + "px";
          }
        }}
