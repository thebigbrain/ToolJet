...
const Preview = ({ darkMode }) => {
  ...
  const previewPanelRef = useRef<HTMLDivElement>();
  ...
  useEffect(() => {
    ...
    setIsJson(
      queryPreviewData !== null && typeof queryPreviewData === "object"
    );
  }, [queryPreviewData]);
  ...
  const renderRawData = () => {
    if (queryPreviewData) {
      return isJson
        ? JSON.stringify(queryPreviewData).toString()
        ...
        >
          <div className="position-relative">
            ...
            <Row className="py-2 border-bottom preview-section-header m-0">
              <Col className="d-flex align-items-center color-slate9">
                Preview
              </Col>
              <Col className="keys text-center d-flex align-items-center">
                <ListGroup
                  className={`query-preview-list-group rounded ${
                    darkMode ? "dark" : ""
                  }`}
                  variant="flush"
