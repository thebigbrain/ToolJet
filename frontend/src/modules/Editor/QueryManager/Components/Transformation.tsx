...
export const Transformation = ({
  ...
  const [lang, setLang] = React.useState(
    options?.transformationLanguage ?? "javascript"
  );
...
return [row for row in data if row['amount'] > 1000]
  ...
  const [enableTransformation, setEnableTransformation] = useState(
  ...
  useEffect(() => {
    if (options.enableTransformation) {
      lang !== (options.transformationLanguage ?? "javascript") &&
        changeOption("transformationLanguage", lang);
      setState({
        ...state,
        [lang]: options.transformation ?? defaultValue[lang],
      });
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  useEffect(() => {
    ...
    if (selectedQueryId !== queryId) {
      const nonLangdefaultCode = getNonActiveTransformations(
        options?.transformationLanguage ?? "javascript"
      );
      const finalState = _.merge(
        {},
        {
          [options?.transformationLanguage ?? lang]:
            options.transformation ?? defaultValue[lang],
        },
        nonLangdefaultCode
      );
  ...
  const labelPopoverContent = (
    <Popover
      ...
      className={`${
        darkMode && "popover-dark-themed theme-dark dark-theme tj-dark-mode"
      } p-0`}
    >
      <p
      ...
      >
        <div className="d-flex flex-fill">
          <OverlayTrigger
            ...
            container={
              document.getElementsByClassName("query-details")[0] as any
            }
          >
            <span
              ...
              style={{
                textDecoration: "underline 2px dashed",
                textDecorationColor: "var(--slate8)",
              }}
            >
              {t(
                "editor.queryManager.transformation.transformations",
                "Transformations"
              )}
            </span>
          </OverlayTrigger>
        ...
        <div className="col flex-grow-1">
          {enableTransformation && (
            <div
              ...
              style={{
                marginBottom: "20px",
                background: `${darkMode ? "#272822" : "#F8F9FA"}`,
              }}
            >
...
const EducativeLabel = ({ darkMode }) => {
  const popoverContent = (
    ...
    >
      <div
        className={`transformation-popover card text-center ${
          darkMode && "tj-dark-mode"
        }`}
      >
        <img
        ...
        <div className="d-flex flex-column card-body">
          ...
          <p className="mb-2">
            <strong style={{ fontWeight: 700, color: "#3E63DD" }}>
              AI copilot
            </strong>{" "}
            helps you write your queries faster. It uses OpenAI&apos;s GPT-3.5
            to suggest queries based on your data.
          </p>

          <Button
            onClick={() =>
              window.open(
                "https://docs.tooljet.com/docs/tooljet-copilot",
                "_blank"
              )
            }
            darkMode={darkMode}
  ...
  return (
    <div className="d-flex">
      ...
      <OverlayTrigger
        ...
        trigger="click"
        placement="right"
        container={document.getElementsByClassName("query-details")[0] as any}
      >
        <span
