...
import { EventManager } from "@/modules/Editor/Inspector/EventManager";
import {
  staticDataSources,
  customToggles,
...
export const QueryManagerBody = ({
  ...
  const ElementToRender = selectedDataSource?.pluginId
    ? source
  ...
  useEffect(() => {
    ...
    defaultOptions.current =
      ...
      JSON.parse(JSON.stringify(selectedQuery?.options));
  ...
  const eventsChanged = (events) => {
    ...
    useDataQueriesStore.getState().actions.saveData({
      ...selectedQuery,
      options: { ...selectedQuery.options, events: events },
    });
  };
      ...
      <div className="d-flex">
        ...
        <div className="query-manager-events pb-4 flex-grow-1">
          <EventManager
            ...
            pages={
              appDefinition?.pages
                ? Object.entries<any>(appDefinition?.pages).map(
                    ([id, page]) => ({
                      ...page,
                      id,
                    })
  ...
  const renderChangeDataSource = () => {
    ...
    return (
      <div
        className={cx("mt-2 d-flex px-4 mb-3", {
          "disabled ": isVersionReleased,
        })}
      >
        <div
          className={`d-flex query-manager-border-color hr-text-left py-2 form-label font-weight-500`}
        >
          Datasource
        </div>
  ...
  return (
    ...
    >
      {selectedQuery?.data_source_id && selectedDataSource !== null
