...
const ToolJetDbOperations = ({
  ...
  const { current_organization_id: organizationId } =
  ...
  const fetchTableInformation = async (table) => {
    const { error, data } = await tooljetDatabaseService.viewTable(
      organizationId,
      table
    );
  ...
  const ComponentToRender: any = getComponent(operation);
