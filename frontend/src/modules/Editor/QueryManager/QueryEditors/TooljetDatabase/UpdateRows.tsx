...
import { TooljetDatabaseContext } from "@/modules/TooljetDatabase/context";
import Select from "@/_ui/Select";
import { operators } from "@/modules/TooljetDatabase/constants";
...
export const UpdateRows = React.memo<UpdateRowsProps>(({ darkMode }) => {
  const { columns, updateRowsOptions, handleUpdateRowsOptionsChange } =
  ...
  function removeColumnOptionsPair(indexedId) {
    ...
    const updatedOptionsObject = Object.keys(existingColumnOption).reduce(
      (acc, key) => {
        if (key !== indexedId) {
  ...
  function addNewColumnOptionsPair() {
    ...
    const existingColumnOption = Object.values
      ? Object.values(updateRowsOptions?.columns)
      : [];
    const emptyColumnOption = { column: "", value: "" };
    handleColumnOptionChange({
      ...existingColumnOption,
      ...{ [uniqueId()]: emptyColumnOption },
    });
  }
  ...
  function addNewFilterConditionPair() {
    const existingFilters = updateRowsOptions?.where_filters
      ? Object.values(updateRowsOptions?.where_filters)
      : [];
    const emptyFilter = { column: "", operator: "", value: "" };
    const newFilter = { ...emptyFilter, ...{ id: uniqueId() } };
    handleWhereFiltersChange({
      ...existingFilters,
  ...
  function removeFilterConditionPair(id) {
    const existingFilters = updateRowsOptions?.where_filters
      ? Object.values(updateRowsOptions?.where_filters)
      : [];
    const updatedFilters = existingFilters.filter((f: any) => f.id !== id);
    const updatedFiltersObject = updatedFilters.reduce((acc, filter: any) => {
  ...
  function updateFilterOptionsChanged(filter) {
    const existingFilters = updateRowsOptions?.where_filters
      ? Object.values(updateRowsOptions?.where_filters)
      : [];
    const updatedFilters = existingFilters.map((f: any) =>
    ...
    <div className="tab-content-wrapper tj-db-field-wrapper mt-2">
      <div className="d-flex mb-2">
        ...
        <div className="field-container flex-grow-1">
          {Object.values<any>(updateRowsOptions?.where_filters || {}).map(
            (filter) => (
              <RenderFilterFields
                key={filter.id}
                {...filter}
          ...
          <ButtonSolid
            ...
            className={`cursor-pointer fit-content ${
              isEmpty(updateRowsOptions?.where_filters) ? "" : "mt-2"
            }`}
          >
            <svg
              width="11"
      ...
      <div className="fields-container d-flex">
        ...
        <div className="field-container flex-grow-1">
          {Object.entries<any>(updateRowsOptions?.columns).map(
            ([key, value]) => {
              return (
                <RenderColumnOptions
                  key={key}
          ...
          {Object.keys(updateRowsOptions?.columns).length !==
            columns.length && (
            <ButtonSolid
              variant="ghostBlue"
              size="sm"
              ...
              className={`cursor-pointer fit-content ${
                isEmpty(updateRowsOptions?.columns) ? "" : "mt-2"
              }`}
            >
              <svg
                width="11"
...
const RenderColumnOptions = ({
  ...
  const existingColumnOptions = Object.values(updateRowsOptions?.columns).map(
    ({ column }) => column
  );
