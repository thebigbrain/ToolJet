...
import ToolJetDbOperations from "@/modules/Editor/QueryManager/QueryEditors/TooljetDatabase/ToolJetDbOperations";
import {
...
const DynamicForm = ({
  ...
  const [currentOrgEnvironmentConstants, setCurrentOrgEnvironmentConstants] =
    React.useState({});
  ...
  React.useEffect(() => {
    if (isGDS) {
      orgEnvironmentConstantService
        .getConstantsFromEnvironment(currentAppEnvironmentId)
        .then((data) => {
          const constants = {};
  ...
  React.useEffect(() => {
    ...
    if (!isEmpty(properties)) {
      ...
      const flipComponentDropdown = find(properties, [
        "type",
  ...
  const getElementProps = ({
    ...
    switch (type) {
      ...
      case "textarea":
        return {
          type,
          placeholder: options?.[key]?.encrypted
            ? "**************"
            : description,
      ...
      case "react-component-oauth-authentication":
        return {
          ...
          access_token_custom_headers:
            options?.access_token_custom_headers?.value,
    ...
    return (
      <div className={`${isHorizontalLayout ? "" : "row"}`}>
        {Object.keys(obj).map((key) => {
          ...
          const Element: any = getElement(type);
            ...
            >
              ...
              >
                <Element
                  ...
                  {...computedProps[key]}
                  data-cy={`${String(label)
                    .toLocaleLowerCase()
                    .replace(/\s+/g, "-")}-text-field`}
  ...
  const isFlipComponentDropdown = (obj) => {
    const flipComponentDropdown = find(obj, [
      "type",
      "dropdown-component-flip",
    ]);
    if (flipComponentDropdown) {
      ...
      const selector =
        options?.[flipComponentDropdown?.key]?.value ||
        options?.[flipComponentDropdown?.key];
      return (
        <>
          <div className={`${isHorizontalLayout ? "" : "row"}`}>
            {flipComponentDropdown.commonFields &&
              getLayout(flipComponentDropdown.commonFields)}
            <div
              className={cx("my-2", {
                "col-md-12":
                  !flipComponentDropdown.className && !isHorizontalLayout,
                "d-flex": isHorizontalLayout,
                "dynamic-form-row": isHorizontalLayout,
                [flipComponentDropdown.className]:
                  !!flipComponentDropdown.className,
              })}
            >
              ...
              >
                <Select
                  ...
                  styles={
                    computeSelectStyles ? computeSelectStyles("100%") : {}
                  }
                  useCustomStyles={computeSelectStyles ? true : false}
                />
              </div>
              {flipComponentDropdown.helpText && (
                <span className="flip-dropdown-help-text">
                  {flipComponentDropdown.helpText}
                </span>
              )}
            </div>
