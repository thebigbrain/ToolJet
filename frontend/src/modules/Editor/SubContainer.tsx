...
import { restrictedWidgetsObj } from "@/modules/Editor/WidgetManager/restrictedWidgetsConfig";
...
export const SubContainer = ({
  ...
  const { enableReleasedVersionPopupState, isVersionReleased } =
    useAppVersionStore(
      (state) => ({
        enableReleasedVersionPopupState:
          state.actions.enableReleasedVersionPopupState,
  ...
  const subContainerHeightRef = useRef<string | number>(height ?? "100%");
      ...
      if (children.length === 0 && addDefaultChildren === true) {
        ...
        defaultChildren.forEach((child) => {
          ...
          const width = layout.width
            ? layout.width
            : (componentMeta.defaultSize.width * 100) / NO_OF_GRIDS;
  ...
  const [, drop] = useDrop(
    () => ({
      ...
      drop(item: any, monitor) {
        ...
        if (
          !restrictedWidgetsObj[parentComp].includes(componentMeta?.component)
        ) {
          const newComponent: any = addNewWidgetToTheEditor(
            componentMeta,
            monitor,
            boxes,
            canvasBoundingRect,
          ...
          toast.error(
            ...
            } is not compatible as a child component of ${parentComp
              .replace(/_/g, " ")
              .toLowerCase()}`,
            {
              style: {
