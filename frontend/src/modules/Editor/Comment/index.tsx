...
import { ItemTypes } from "@/modules/Editor/ItemTypes";
import CommentHeader from "@/modules/Editor/Comment/CommentHeader";
import CommentBody from "@/modules/Editor/Comment/CommentBody";
import CommentFooter from "@/modules/Editor/Comment/CommentFooter";
...
const Comment = ({
  ...
  const [open, trigger, content, setOpen]: any = usePopover(false);
  const [, drag] = useDrag(() => ({
  ...
  const searchUser = (query, callback) => {
    ...
    organizationService
      ...
      .then((data) =>
        data.users.map((u) => ({
          id: u.user_id,
          display: `${capitalize(u.first_name ?? "")} ${capitalize(
            u.last_name ?? ""
          )}`,
          email: u.email,
