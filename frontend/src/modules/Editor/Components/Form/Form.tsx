...
import { SubCustomDragLayer } from "@/modules/Editor/SubCustomDragLayer";
import { SubContainer } from "@/modules/Editor/SubContainer";
// eslint-disable-next-line import/no-unresolved
import { diff } from "deep-object-diff";
import _, { omit } from "lodash";
import { Box } from "@/modules/Editor/Box";
...
export const Form = function Form(props) {
  ...
  const { visibility, disabledState, borderRadius, borderColor, boxShadow } =
    styles;
  const { buttonToSubmit, loadingState, advanced, JSONSchema } = properties;
  const backgroundColor =
    ["#fff", "#ffffffff"].includes(styles.backgroundColor) && darkMode
      ? "#232E3C"
  ...
  const checkJsonChildrenValidtion = () => {
    const isValid = Object.values<any>(childrenData).every(
    ...
    if (advanced) {
      ...
      Object.keys(childComponents).forEach((childId) => {
        if (childrenData[childId]?.name) {
          formattedChildData[childrenData[childId].name] = omit(
            childrenData[childId],
            "name"
          );
          childValidation =
            childValidation && (childrenData[childId]?.isValid ?? true);
        }
      });
    }
    formattedChildData = Object.fromEntries(
      // eslint-disable-next-line no-unused-vars
      Object.entries<any>(formattedChildData).map(
        ([key, { keyValue, ...rest }]) => [key, rest]
      )
    );
  ...
  useEffect(() => {
    ...
    Object.entries<any>(currentState.components).forEach(([name, value]) => {
      if (
        childIds.includes(value.id) &&
        name !== childrenData[value.id]?.name
      ) {
    ...
    if (
      Object.keys(
        (diff(childrenData, childDataRef.current) as any).length !== 0
      )
    ) {
  ...
  useEffect(() => {
    document.addEventListener("submitForm", handleFormSubmission as any);
    return () => {
      document.removeEventListener("submitForm", handleFormSubmission as any);
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const handleFormSubmission = ({ detail: { buttonComponentId } }) => {
    if (!advanced) {
      ...
      buttonComponentId == uiComponents.length - 1 &&
      JSONSchema.hasOwnProperty("submitButton")
    ) {
      fireSubmissionEvent();
    }
  };
  function onComponentOptionChangedForSubcontainer(
    ...
    return containerProps.onComponentOptionChanged(
      component,
      optionName,
      value
    );
  ...
  const onOptionChange = ({ component, optionName, value, componentId }) => {
    let keyValue =
      JSONSchema?.properties && findKeyByLabel(JSONSchema.properties, value);
  ...
  return (
    <form
      ...
      onClick={(e) => {
        if ((e.target as HTMLElement).className === "real-canvas")
                ...
                >
                  <Box
                    ...
                    onComponentOptionsChanged={
                      containerProps.onComponentOptionsChanged
                    }
