...
export const Listview = function Listview({
  ...
  const {
    ...
    mode = "list",
    columns = 1,
  }: any = { ...fallbackProperties, ...properties };
  const { visibility, disabledState, borderRadius, boxShadow }: any = {
    ...fallbackStyles,
    ...styles,
  };
  ...
  const startIndexOfRowInThePage =
    currentPage === 1 ? 0 : currentPage * rowPerPageValue - rowPerPageValue;
    ...
    >
      <div
        className={`row w-100 m-0 ${
          enablePagination && "pagination-margin-bottom-last-child"
        }`}
      >
        {filteredData.map((listItem, index) => (
          <div
            className={`list-item ${mode == "list" && "w-100"}  ${
              showBorder && mode == "list" ? "border-bottom" : ""
            }`}
            style={{
              position: "relative",
              height: `${rowHeight}px`,
            ...
            <SubContainer
              ...
              onOptionChange={function ({ component, optionName, value }) {
                setChildrenData((prevData) => {
                  const changedData = {
                    [component.name]: { [optionName]: value },
                  };
