...
export const Modal = function Modal({
  ...
  useEffect(() => {
    const handleModalOpen = () => {
      ...
      const modalBackdropEl: any =
        document.getElementsByClassName("modal-backdrop")[0];
      const realCanvasEl: any =
  ...
  const backwardCompatibilityCheck =
    height == "34" || modalHeight != undefined ? true : false;

  const customStyles = {
    modalBody: {
      height: backwardCompatibilityCheck ? modalHeight : height,
      backgroundColor:
        ["#fff", "#ffffffff"].includes(bodyBackgroundColor) && darkMode
          ? "#1F2837"
          : bodyBackgroundColor,
      overflowX: "hidden",
      overflowY: "auto",
    ...
    modalHeader: {
      backgroundColor:
        ["#fff", "#ffffffff"].includes(headerBackgroundColor) && darkMode
          ...
          : headerBackgroundColor,
      color:
        ["#000", "#000000", "#000000ff"].includes(headerTextColor) && darkMode
          ? "#fff"
          : headerTextColor,
    },
    buttonStyles: {
      ...
      "--tblr-btn-color-darker": tinycolor(triggerButtonBackgroundColor)
        .darken(8)
        .toString(),
      boxShadow,
    },
  ...
  useEffect(() => {
    if (closeOnClickingOutside) {
      const handleClickOutside = (event) => {
        const modalRef =
          parentRef.current.parentElement.parentElement.parentElement;
