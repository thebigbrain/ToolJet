...
export const Calendar = function ({
  ...
  const events = Array.isArray(properties?.events)
    ? properties?.events?.map((event) =>
        prepareEvent(event, properties.dateFormat)
      )
    : [];
  ...
  const startTime = properties.startTime
    ? parseDate(properties.startTime, properties.dateFormat)
    : todayStartTime;
  const endTime = properties.endTime
    ? parseDate(properties.endTime, properties.dateFormat)
    : todayEndTime;

  const [currentDate, setCurrentDate] = useState(defaultDate);
  const [eventPopoverOptions, setEventPopoverOptions] = useState<any>({
    show: false,
  });
  ...
  const eventPropGetter = (event) => {
    ...
    const textStyle =
      event.textOrientation === "vertical" &&
      exposedVariables.currentView != "month"
  ...
  const slotSelectHandler = (calendarSlots) => {
    ...
    const formattedSlots = slots.map((slot) =>
      moment(slot).format(properties.dateFormat)
    );
  ...
  useEffect(() => {
    ...
    if (
      defaultDate !== null &&
      !_.isEqual(exposedVariables.currentDate, properties.defaultDate)
    ) {
      setExposedVariable(
        "currentDate",
        moment(defaultDate).format(properties.dateFormat)
      );
  ...
  const components = {
    ...
    week: {
      header: (props) => (
        <div>{moment(props.date).format(styles.weekDateFormat)}</div>
      ),
    },
  };
