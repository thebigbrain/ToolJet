...
export const Tabs = function Tabs({
  ...
  const widgetVisibility =
    component.definition.styles?.visibility?.value ?? true;
  const disabledState =
    component.definition.styles?.disabledState?.value ?? false;
  const defaultTab = component.definition.properties.defaultTab.value;
  // config for tabs. Includes title
  const tabs = isExpectedDataType(
    resolveReferences(component.definition.properties.tabs.value, currentState),
    "array"
  );
  let parsedTabs = tabs;
  ...
  const renderOnlyActiveTab =
    component.definition.properties?.renderOnlyActiveTab?.value ?? false;
  ...
  const highlightColor =
    component.definition.styles?.highlightColor?.value ?? "#f44336";
  ...
  parsedDefaultTab = resolveWidgetFieldValue(parsedDefaultTab, currentState, [
    1,
  ]);

  const parsedDisabledState =
    typeof disabledState !== "boolean"
      ? resolveWidgetFieldValue(disabledState, currentState)
      : disabledState;

  const parsedHideTabs =
    typeof hideTabs !== "boolean"
      ? resolveWidgetFieldValue(hideTabs, currentState)
      : hideTabs;
  ...
  try {
    parsedWidgetVisibility = resolveReferences(
  ...
  registerAction(
    ...
    async function (id) {
      if (id) {
        setCurrentTab(id);
        setExposedVariable("currentTab", id).then(() =>
        ...
        {parsedTabs.map((tab) => (
          <li
            ...
            onClick={() => {
              ...
              !tab?.disabled &&
                setExposedVariable("currentTab", tab.id).then(() =>
                  fireEvent("onTabSwitch")
                );
            }}
            key={tab.id}
          >
            <a
              ...
              style={{
                ...
                borderBottom:
                  currentTab == tab.id && `1px solid ${parsedHighlightColor}`,
