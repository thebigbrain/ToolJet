...
import { createWebsocketConnection } from "@externals/helpers/websocketConnection";
import RealtimeCursors from "@/modules/Editor/RealtimeCursors";
import { initEditorWalkThrough } from "@externals/helpers/createWalkThrough";
...
class EditorComponent extends React.Component<EditorProps, EditorState> {
  renameQueryNameId: React.Ref<number>;
  dataSourceModalRef: React.RefObject<unknown>;
  canvasContainerRef: React.RefObject<HTMLDivElement>;
  selectionRef: React.RefObject<Selecto>;
  selectionDragRef: React.MutableRefObject<boolean>;
  queryManagerPreferences: any;
  autoSave: any;
  realtimeSave: any;
  appDataStoreListner: any;
  dataQueriesStoreListner: any;
  currentVersion: { [x: number]: number };
  currentVersionChanges: {};
  noOfVersionsSupported: number;
  canUndo: boolean;
  canRedo: boolean;
  socket: WebSocket;
  ...
  constructor(props) {
    ...
    const defaultPageId: string = uuid();
  ...
  fetchApp = (startingPageHandle?: PageHandle) => {
    const appId = this.props.params.id;

    const callBack = async (data) => {
      let dataDefinition = defaults(data.definition, this.defaultDefinition);

      const pages = Object.entries<Page>(dataDefinition.pages).map(
  ...
  handleRedo = () => {
    if (this.canRedo) {
      let currentVersion: VersionId =
        this.currentVersion[this.state.currentPageId];

      const appDefinition: ApplicationDefinition = applyPatches(
        this.state.appDefinition,
  ...
  appDefinitionChanged = (
    ...
    opts: ApplicationDefinitionOption = {}
  ) => {
  ...
  handleComponentClick = (id: JetComponentId, component: JetViewComponent) => {
    this.setState({
  ...
  executeDeletepageRequest = () => {
    ...
    const pages = new Map(this.state.appDefinition.pages);
    pages.delete(pageId);

    const newAppDefinition: ApplicationDefinition = {
      ...this.state.appDefinition,
      pages,
    };
  ...
  clonePage = (pageId) => {
    ...
    if (!isEmpty(currentPage?.components)) {
      ...
      Object.values(newPageData.components).map((comp: JetComponentEntity) => {
        if (comp.parent) {
    ...
    return (
      <div className="editor wrapper">
        ...
        <Confirm
          ...
          darkMode={this.props.darkMode}
          queryConfirmationData={undefined}
        />
