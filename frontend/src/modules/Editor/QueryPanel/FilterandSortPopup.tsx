...
const FilterandSortPopup = ({
  ...
  const [showMenu, setShowMenu] = useShowPopover(
    false,
    "#query-sort-filter-popover",
    "#query-sort-filter-popover-btn"
  );
  ...
  useEffect(() => {
    if (showMenu) {
      ...
      const createdSources = dataQueries.map((query) => {
        const globalDS = [...dataSources, ...globalDataSources].find(
      ...
      setSources(
        createdSources
          ...
          .sort((a, b) => {
            const aChecked = selectedDataSources.some(
              (item) => item.id === a.id && item.kind === a.kind
            );
            const bChecked = selectedDataSources.some(
              (item) => item.id === b.id && item.kind === b.kind
            );
      ...
      default:
        return (
          ...
          >
            ...
            <div
              className={`tj-list-btn mx-1 ${
                selectedDataSources.length ? "d-flex" : ""
              }`}
            >
              <MenuButton
  ...
  return (
    ...
    >
      <span>
        ...
        >
          <Filter width="13" fill="var(--slate12)" />
      ...
      >
        {sources.map((source) => (
          <label
            ...
            role="button"
            htmlFor={`default-${source.id || source.kind}`}
          >
            <Form.Check // prettier-ignore
              type={"checkbox"}
              id={`default-${source.id || source.kind}`}
              onChange={(e) =>
                onFilterDatasourcesChange(source, e.target.value)
              }
              className="m-0"
  ...
  return (
    <div className={`field p-2 ${noMargin ? "" : "mx-1"} tj-list-btn`}>
      ...
      >
        ...
        {active && (
          <Tick width="20" viewBox="0 0 22 22" fill="var(--indigo9)" />
        )}
      </Button.UnstyledButton>
    </div>
  );
