...
const QueryPanel = ({
  ...
  const [isExpanded, setExpanded] = useState(
    queryManagerPreferences.current?.isExpanded ?? true
  );
  ...
  const [_windowSize, isWindowResizing] = useWindowResize();
  const windowSize = _windowSize as any;

  useEffect(() => {
    const queryPanelStoreListner = useQueryPanelStore.subscribe(
      ({ selectedQuery }, prevState) => {
  ...
  const onMouseMove = (e) => {
    if (queryPaneRef.current) {
      const componentTop = Math.round(
        queryPaneRef.current.getBoundingClientRect().top
      );
      ...
      if (isDragging) {
        ...
        localStorage.setItem(
          ...
          JSON.stringify(queryManagerPreferences.current)
        );
