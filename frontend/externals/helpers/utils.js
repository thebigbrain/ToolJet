...
export function resolve(data, state) {
  if (
    data.startsWith("{{queries.") ||
    data.startsWith("{{globals.") ||
    data.startsWith("{{components.")
  ) {
...
export function resolveWidgetFieldValue(
  prop,
  state,
  _default = [],
...
export function validateWidget({
  ...
  const validationRegex = resolveWidgetFieldValue(
    regex,
    currentState,
    "",
    customResolveObjects
  ...
  const resolvedMinLength = resolveWidgetFieldValue(
    minLength,
    currentState,
    0,
    customResolveObjects
  ...
  const resolvedMaxLength = resolveWidgetFieldValue(
    maxLength,
    currentState,
    undefined,
    customResolveObjects
  ...
  const resolvedMinValue = resolveWidgetFieldValue(
    minValue,
    currentState,
    undefined,
    customResolveObjects
  ...
  const resolvedMaxValue = resolveWidgetFieldValue(
    maxValue,
    currentState,
    undefined,
    customResolveObjects
  ...
  const resolvedCustomRule = resolveWidgetFieldValue(
    customRule,
    currentState,
    false,
    customResolveObjects
...
export const getuserName = (formData) => {
  ...
  if (nameArray?.length > 0)
    return `${nameArray?.[0][0]}${
      nameArray?.[1] != undefined && nameArray?.[1] != ""
        ? nameArray?.[1][0]
        : ""
    } `;
  return "";
...
export const getWorkspaceIdFromURL = () => {
  ...
  const workspaceId = subpath
    ? pathnameArray[subpathArray.length]
    : pathnameArray[0];
...
export const handleHttpErrorMessages = (
  { statusCode, error },
  feature_name
) => {
  switch (statusCode) {
...
export const isQueryRunnable = (query) => {
  ...
  return !!(
    ...
    query?.dataSourceId ||
    !isEmpty(query?.plugins)
  );
};

export const redirectToDashboard = () => {
  const subpath = getSubpath();
  navigateTo(`${subpath ? `${subpath}` : ""}/${getWorkspaceId()}`);
};
